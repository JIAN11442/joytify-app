...
const SECRET_NAME = process.env.SECRET_NAME || "MERN_JOYTIFY_ENVS";
...
class TestDataGenerator {
  ...
  generateTestPlayback(testUserId, templatePlayback, showProgress = false) {
    ...
    for (let i = 0; i < PLAYBACK_RECORDS_PER_USER; i++) {
      ...
      if (i < oldRecordsCount) {
        ...
        recordTime = recentStartTime + ((i - oldRecordsCount) / recentRecordsCount) * recentDataRange;
      }
  ...
  async generateTestData(count, appendMode = false) {
    ...
    console.log(`   Records older than 60 days: ${totalOldRecords.toLocaleString()} (${(oldRecordsCount/PLAYBACK_RECORDS_PER_USER*100).toFixed(1)}%)`);
    console.log(`   Records within 30 days: ${totalRecentRecords.toLocaleString()} (${(recentRecordsCount/PLAYBACK_RECORDS_PER_USER*100).toFixed(1)}%)`);
    console.log(`   🗑️  Expected cleanup: ${totalOldRecords.toLocaleString()} records will be deleted by 60-day cleanup`);
    console.log(`   ✅ Expected remaining: ${totalRecentRecords.toLocaleString()} records will remain after cleanup`);
  }

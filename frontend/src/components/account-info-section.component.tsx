...
const AccountInfoSection: React.FC<AccountInfoSectionProps> = ({ profileUser }) => {
  const { authForThirdParty: isThirdPartyUser } = profileUser;

  const { fm } = useScopedIntl();
  const toastSettingsAccountFm = fm("toast.settings.account");
  const accountInfoSectionFm = fm("settings.account.infoSection");
